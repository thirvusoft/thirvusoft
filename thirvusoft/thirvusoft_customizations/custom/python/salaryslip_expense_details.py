...
class SalarySlip(TransactionBase):
	...
	def set_net_pay(self):
		...
		self.net_pay = (flt(self.gross_pay)+flt(self.ts_total_reimbursement)) - (flt(self.total_deduction) + flt(self.total_loan_repayment))
	...
	def calculate_total_for_salary_slip_based_on_timesheet(self):
		...
		self.net_pay = (flt(self.gross_pay)+flt(self.ts_total_reimbursement)) - flt(self.total_deduction)
...
def salaryslip(doc):
	...
	doc=Exp(**doc)
	expense_claim=frappe.db.get_all("Expense Claim Detail",fields=['expense_type','amount','sanctioned_amount','parent'], filters={'expense_date':['between',(doc.start_date,doc.end_date)]})
	match=[i['parent'] for i in expense_claim if(frappe.get_value('Expense Claim',i['parent'],'employee') == doc.employee)]
	sum=[]
	types=[]
	main_types={i['expense_type']:0 for i in expense_claim }
	for i in list(set(match)):
		types=main_types
		for j in expense_claim:
			if(i == j['parent']):
				types[j['expense_type']]+=j['amount']
	sum.append(types)
	return sum 
